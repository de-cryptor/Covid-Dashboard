...
class Oxygen(View):
    ...
    def get(self,request):
        ...
        return render(request, 'resources.html', {'type' : 'oxygen', 'page_title' : 'Oxygen Availability Leads on Twitter' , 'data': data.get('data', [])})
        
class Plasma(TemplateView):
    def get(self,request):
        query = request.GET.get('query', 'covid blood plasma');
        data = fetch_tweets(query)
        return render(request, 'resources.html', {'type' : 'plasma', 'page_title' : 'Plasma Availability Leads on Twitter' , 'data': data.get('data', [])})
        
class Beds(TemplateView):
    def get(self,request):
        query = request.GET.get('query', 'covid beds');
        data = fetch_tweets(query)
        return render(request, 'resources.html', {'type' : 'beds', 'page_title' : 'Beds Availability Leads on Twitter' , 'data': data.get('data', [])})
        
class Medicines(TemplateView):
    def get(self,request):
        query = request.GET.get('query', 'covid medicines');
        data = fetch_tweets(query)
        return render(request, 'resources.html', {'type' : 'medicines', 'page_title' : 'Medicines Availability Leads on Twitter' , 'data': data.get('data', [])})
